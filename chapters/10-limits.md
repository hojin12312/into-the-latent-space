...
```
[ AI 가 할 수 있는 것 ]            [ AI 가 못 하는 것 ]

  글 만들기                          시간이 흐른 걸 알기
  학습한 패턴 우려내기                 자기가 무슨 모델인지 정확히 알기
  빠른 초안 쓰기                     자기 답이 사실인지 검증하기
  번역, 요약, 정리                   자기 한계를 자발적으로 인지하기
  도구 호출 (에이전트일 때)            잘못된 답에 책임지기
                                   마음을 갖기 (현재로선)
```
...
이 책 전체에서 우리는 AI 를 "흉내쟁이", "알바생", "비서" 같은 사람의 비유로 풀어왔어요. 그 비유들은 이해를 돕기 위한 출발점이에요. 정확히 말하면 AI 는 사람도 아니고, 도구도 아니고, 그 사이의 어떤 새로운 무엇이에요. 우리에게는 아직 그 새로운 무엇을 정확히 부를 단어가 없어요. 그래서 우리는 비유로 더듬어 가요. 이 책의 여정도 그런 더듬어가기였어요.
...
10편의 본편이 모두 끝났어요. 이제 마지막 한 화 — 우리가 함께 걸어온 길을 돌아보면서, 책 제목에 숨어 있던 "latent space" 의 정체를 풀어드릴게요.
