...
네 번째로 — 가장 깊은 곳에 깔린 함정은 **잘못된 산수**예요. "AI 가 일을 가져간다" 는 말은, 알게 모르게 "세상에 일거리의 총량은 정해져 있다" 는 가정 위에 서 있어요. 한 사람이 두 배로 빨라지면 누군가 한 명은 잘려야 한다는 결론은, 이 (잘못된) 가정 위에서만 성립해요. 경제학은 이걸 **노동 총량의 오류**(Lump of Labor Fallacy, 일거리의 총량이 고정되어 있다고 가정하는 인지 오류) 라고 불러요.

코드가 가장 또렷한 사례예요. "AI 가 코드를 빠르게 짜주니 개발자 일이 줄어든다" 는 결론은, 세상에 필요한 코드의 양이 정해져 있다는 전제 위에서만 성립해요. 그런데 세상은 그렇지 않아요. 아직 소프트웨어가 닿지 않은 분야가 어마어마해요 — 소상공인 행정, 농업 데이터, 지역 행정, 비영리 단체의 도구, 1인 사업자 자동화, 의료 기록 정리. 이미 닿은 분야조차 자율주행·로봇·AI 자체·생명과학처럼 훨씬 더 복잡한 코드가 얹어질 여지가 끝없이 남아 있고요. 코드 한 줄의 단가가 떨어지면, 새로 가능해지는 코드의 양이 폭발해요. 19세기 영국에서 석탄을 더 효율적으로 쓰게 됐을 때 석탄 소비가 폭발했던 — **제번스의 역설**(Jevons paradox, 자원이 효율화될수록 총사용량이 오히려 늘어나는 현상) — 과 정확히 같은 패턴이에요. 종말론은 이 폭발을 보지 않은 채, 분자만 가지고 분수를 계산해요.
